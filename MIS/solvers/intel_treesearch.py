...
class IntelTreesearch(MWISSolver):
    ...
    def _prepare_instance(source_instance_file: pathlib.Path, cache_directory: pathlib.Path, **kwargs):
        ...
        try:
            ...
            return # we don't have a valid graph
    ...
    def train(self, train_data_path: pathlib.Path, results_path: pathlib.Path, parameters):
        ...
        if "dimes" in parameters.keys() and parameters["dimes"]:
            script_file = self.directory() / "NPHard" / "train_dimes_batched.py"
            # if self.model_state_path and "large" in str(results_path):
            #     script_file = self.directory() / "NPHard" / "train_dimes.py"
            if "meta_update" in parameters.keys():
                arguments += ["--meta_update"]
            if "meta_steps" in parameters.keys():
                arguments += ["--meta_steps", str(parameters["meta_steps"])]
            if "ppo" in parameters.keys():
                arguments += ["--ppo"]
            if "random_prune" in parameters.keys():
                arguments += ["--random_prune"]
            if "aux_grad" in parameters.keys():
                arguments += ["--aux_grad"]
            if "normalize_factor" in parameters.keys():
                arguments += ["--normalize_factor", str(parameters["normalize_factor"])]
            if "supervision_factor" in parameters.keys():
                arguments += ["--supervision_factor", str(parameters["supervision_factor"])]
    ...
    def solve(self, solve_data_path: pathlib.Path, results_path: pathlib.Path, parameters):
        ...
        if parameters["num_threads"] == 1:
            ...
            # script_file = self.directory() / "NPHard" / "demo.py"
            # if "dimes" in parameters.keys() and parameters["dimes"]:
            script_file = self.directory() / "NPHard" / "demo_dimes.py"
            if "dimes" in parameters.keys() and parameters["dimes"]:
                arguments += ["--dimes"]
            else:
                arguments += ["--hidden1", str(32)]
            if "meta_update" in parameters.keys():
                arguments += ["--meta_update"]
            if "meta_steps" in parameters.keys():
                arguments += ["--meta_steps", str(parameters["meta_steps"])]
            ...
            raise NotImplementedError("Multi-threaded solver not implemented yet.")
        ...
        if "random_heatmap" in parameters.keys():
            arguments += ["--random_heatmap"]
