...
def _train_or_solve(args):
    ...
    if args.operation == "solve":
        ...
        if args.dimes:
            parameters["dimes"] = "yes"

        if args.meta_update:
            parameters["meta_update"] = "yes"

        if args.meta_steps:
            parameters["meta_steps"] = args.meta_steps
        ...
        if args.random_heatmap:
            parameters["random_heatmap"] = "yes"

        if args.normalize_factor:
            parameters["normalize_factor"] = args.normalize_factor
        ...
        if args.random_prune:
            parameters["random_prune"] = "yes"

        if args.aux_grad:
            parameters["aux_grad"] = "yes"

        if args.normalize_factor:
            parameters["normalize_factor"] = args.normalize_factor

        if args.supervision_factor:
            parameters["supervision_factor"] = args.supervision_factor
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dimes", action="store_true", default=False, help="If enabled, train intel_treesearch with DIMES.")
    parser.add_argument("--random_heatmap", action="store_true", default=False, help="If enabled, use random heatmap for intel_treesearch")
    ...
    train_parser.add_argument("--meta_update", action="store_true", default=False, help="If enabled, train intel_treesearch with DIMES and meta learning.")
    train_parser.add_argument("--meta_steps", type=int, action="store", default=1, help="Number of meta steps to train for.")
    train_parser.add_argument("--ppo", action="store_true", default=False, help="If enabled, train intel_treesearch with DIMES and PPO.")
    train_parser.add_argument("--random_prune", action="store_true", default=False, help="If enabled, train intel_treesearch with DIMES and random pruning.")
    train_parser.add_argument("--aux_grad", action="store_true", default=False, help="If enabled, train intel_treesearch with DIMES and auxiliary gradient.")
    train_parser.add_argument("--normalize_factor", type=float, default=2.0,
                              help="Normalization factor for the reward.")
    train_parser.add_argument("--supervision_factor", type=float, default=0.0, help="Supervision factor for the reward.")
    # Lwd Arguments
    ...
    solve_parser.add_argument("--meta_update", action="store_true", default=False,
                              help="If enabled, solve intel_treesearch with DIMES and meta learning.")
    solve_parser.add_argument("--meta_steps", type=int, action="store", default=1, help="Number of meta steps to train for.")
    solve_parser.add_argument("--normalize_factor", type=float, default=2.0,
                              help="Normalization factor for the reward.")
